...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport {
	...
	private final int requestcode1 = 4581;
	// private EditText mPasswordVie458w;
	...
	public void createUI(Bundle savedInstanceState) {
		...
		mDocView = new MuPDFReaderView(this) {
			...
			protected void onMoveToChild(int i) {
				...
				getPageNumber(i);
		      //  int s= i-1;
				//bookMark.setBackgroundColor(R.drawable.abc_ab_share_pack_holo_dark);
			...
			public void getPageNumber(int i)
			{
				...
				for(int j = 0;j<bookMarks.size();j++){
			        	 if(bookMarks.get(j).getPageId().equals(i+""))
			        	 {
			        		 check=1;
			        		 Log.d("page No. is  ",i+"");
			        		break;
				...
				if(check==1)
				{
					 bookMark.setText("UnMark");
				}
				else if(check==2){
					 bookMark.setText("Mark");
				}
			}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		switch (requestCode) {
			...
			case  requestcode1:
				...
				int pageNum = Integer.parseInt(result);
	...
	*/
	...
	private void makeButtonsView() {
		...
		try{
				...
				bookMark = (Button)mButtonsView.findViewById(R.id.btnbookMark);
				bookMarkList=(Button)mButtonsView.findViewById(R.id.btnlist);
...
//
	public void onClickList(View v) {
		...
		Intent mlist = new Intent(this,BookMarkList.class);
		//startActivity(mlist);
		startActivityForResult(mlist, requestcode1);
		/////////////mTopBarMode = TopBarMode.More;
		/////////////mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
	}
	public void onBookMark(View v) {
		db= new DbHandler(this);
		BookMark mCurrency = new BookMark();
		if(bookMark.getText().toString().equals("Mark")  )
		{
			mCurrency.setPageId(pageNumber+"");
			Log.d("page in main", pageNumber+"");
			db.addBookMark(mCurrency);
			
		bookMark.setText("UnMark");
		}
		else if(bookMark.getText().toString().equals("UnMark"))
		{
			//mCurrency.setId(pageNumber);
			db.deleteBookMark(pageNumber);//(pageNumber);
			//bookMarks = db.getPages();
			bookMark.setText("Mark");
		}
